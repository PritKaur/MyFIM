...
def load_monitored_file_path():
    ...
    return config.get("monitored_path")
    
#Saves the detected file change to alerts.json
def save_detection_alert(file_path, change_type, dest_path=None):
    alerts = []
...
class MyFIMEventHandler(FileSystemEventHandler):
    def __init__(self, baseline, monitored_path):
        #This will store the baseline and the path being monitored
        self.baseline = baseline
        self.monitored_path = monitored_path
        #This tracks the last known hash per file to prevent duplicate alerts 
    ...
    def on_moved(self, event):
        ...
        try:
            ...
            baseline_hash = self.baseline.get(os.path.normpath(old_path))
